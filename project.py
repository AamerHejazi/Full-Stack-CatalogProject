...
APPLICATION_NAME = "Pharmacy Menu Application"

# Connect to Database and create database session
engine = create_engine(
    'sqlite:///pharmacymenuwithusers.db?check_same_thread=False')
...
# JSON APIs to view Pharmacy Information
@app.route('/Pharmacy/<int:pharmacy_id>/menu/JSON')
def pharmacyMenuJSON(pharmacy_id):
    pharmacy = session.query(Pharmacy).filter_by(id=pharmacy_id).one()
    items = session.query(MenuItem).filter_by(pharmacy_id=pharmacy_id).all()
    return jsonify(MenuItems=[i.serialize for i in items])


@app.route('/pharmacy/<int:pharmacy_id>/menu/<int:menu_id>/JSON')
def menuItemJSON(pharmacy_id, menu_id):
    Menu_Item = session.query(MenuItem).filter_by(id=menu_id).one()
    return jsonify(Menu_Item=Menu_Item.serialize)


@app.route('/pharmacy/JSON')
def pharmaciesJSON():
...
def showPharmacies():
    pharmacies = session.query(Pharmacy).order_by(asc(Pharmacy.name))
    if 'username' not in login_session:
        return render_template('publicpharmacies.html', pharmacies=pharmacies)
    else:
...
def editPharmacy(pharmacy_id):
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('Pharmacy Successfully Edited %s' % editedPharmacy.name)
            return redirect(url_for('showPharmacies'))
    else:
        return render_template('editPharmacy.html', pharmacy=editedPharmacy)
...
def deletePharmacy(pharmacy_id):
    ...
    if request.method == 'POST':
        ...
        flash('%s Successfully Deleted' % pharmacyToDelete.name)
        session.commit()
        return redirect(url_for('showPharmacies', pharmacy_id=pharmacy_id))
    else:
        return render_template(
            'deletePharmacy.html', pharmacy=pharmacyToDelete)


# Show a pharmacy menu


@app.route('/pharmacy/<int:pharmacy_id>/')
@app.route('/pharmacy/<int:pharmacy_id>/menu/')
def showMenu(pharmacy_id):
    pharmacy = session.query(Pharmacy).filter_by(id=pharmacy_id).one()
    items = session.query(MenuItem).filter_by(pharmacy_id=pharmacy_id).all()
    creator = getUserInfo(pharmacy.user_id)
    if 'username' not in login_session:
        return render_template(
            'publicmenu.html', items=items, pharmacy=pharmacy, creator=creator)
    elif creator.id != login_session['user_id']:
        return render_template(
            'publicmenu.html', items=items, pharmacy=pharmacy, creator=creator)
    else:
        return render_template(
            'menu.html', items=items, pharmacy=pharmacy, creator=creator)
...
@app.route(
    ...
    methods=['GET', 'POST'])
def editMenuItem(pharmacy_id, menu_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'editmenuitem.html',
            pharmacy_id=pharmacy_id, menu_id=menu_id, item=editedItem)


# Delete a menu item
@app.route(
    '/pharmacy/<int:pharmacy_id>/menu/<int:menu_id>/delete',
    methods=['GET', 'POST'])
def deleteMenuItem(pharmacy_id, menu_id):
    pharmacy = session.query(Pharmacy).filter_by(id=pharmacy_id).one()
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'deletemenuitem.html', itemToDelete=itemToDelete)
